...
function updategroupBlogEndpoint($postData, $fileData)
{
    ...
    // ---------------------------------------------------
    // 1️⃣ VALIDATION (group ONLY version)
    // ---------------------------------------------------
    if (empty($blogId) || empty($group_id) || empty($title) || empty($article)) {
        return ['status' => 'error', 'messages' => generateMessage("All required fields must be filled, including group_id.", "red")];
    }

    // ---------------------------------------------------
    // 2️⃣ FETCH EXISTING BLOG
    // ---------------------------------------------------
    $existing = mysqli_query($con, "
        SELECT status, user_id, title 
        FROM {$siteprefix}forums 
        WHERE id = '$blogId' AND group_id = '$group_id'
    ");

    if (!$existing || mysqli_num_rows($existing) === 0) {
        return ['status' => 'error', 'messages' => generateMessage("Blog not found for this group.", "red")];
    }

    $blog       = mysqli_fetch_assoc($existing);
    $oldStatus  = $blog['status'];
    $authorId   = $blog['user_id'];  // Poster = Author only
    $oldTitle   = $blog['title'];
    ...
    // ---------------------------------------------------
    // 4️⃣ REGENERATE SLUG ONLY IF TITLE CHANGES
    // ---------------------------------------------------
    $slugSql = '';
    if ($oldTitle !== $title) {
        $baseSlug = strtolower(trim(preg_replace('/[^a-z0-9]+/i', '-', $title), '-'));
    ...
    if (!mysqli_query($con, $query)) {
        return ['status' => 'error', 'messages' => generateMessage("DB Error: " . mysqli_error($con), "red")];
    }

    // ---------------------------------------------------
    // 7️⃣ FETCH GROUP INFO
    // ---------------------------------------------------
    $groupQ = mysqli_query($con, "
        SELECT user_id, group_name, slug 
        FROM {$siteprefix}groups 
        WHERE id='$group_id' LIMIT 1
    ");

    $group = $groupQ ? mysqli_fetch_assoc($groupQ) : null;
    $groupCreatorId = $group['user_id'] ?? 0;
    ...
    // ---------------------------------------------------
    // 8️⃣ FETCH AUTHOR INFO
    // ---------------------------------------------------
    $authorQuery = mysqli_query($con, "
        SELECT first_name, email 
        FROM {$siteprefix}users 
        WHERE id = '$authorId' LIMIT 1
    ");

    $author = $authorQuery ? mysqli_fetch_assoc($authorQuery) : null;
    $authorName  = $author['first_name'] ?? '';
    $authorEmail = $author['email'] ?? '';

    // ---------------------------------------------------
    // 9️⃣ NOTIFICATIONS (GROUP ONLY)
    // ---------------------------------------------------
    ...
    if ($oldStatus !== 'active' && $status === 'active') {
        sendEmail(
            ...
            $siteMail,
            $authorName,
            "<p>Your blog <strong>{$oldTitle}</strong> has been approved.</p>",
    ...
    if ($status === 'rejected') {
        sendEmail(
            ...
            $siteMail,
            $authorName,
            "<p>Your blog <strong>{$oldTitle}</strong> was rejected.</p>
    ...
    // ---------------------------------------------------
    // 🔥 GROUP-WIDE NOTIFICATIONS WHEN ACTIVE
    // ---------------------------------------------------
    if ($status === 'active') {
        ...
        while ($m = mysqli_fetch_assoc($membersQ)) {
            sendEmail(
                ...
                $siteMail,
                $m['first_name'],
                "A new blog was posted in <b>{$group_name}</b>: <b>{$title}</b><br><br>
                 <a href='{$groupUrl}'>Visit group</a>",
                "New Blog in Group"
            );

            insertAlert($con, $m['user_id'], "New blog: '{$title}' posted.", date("Y-m-d H:i:s"), 0);
        }
        ...
        if ($groupCreatorId) {
            sendEmail(
                ...
                $siteMail,
                $group['first_name'] ?? '',
                "A new blog <b>{$title}</b> was posted in your group <b>{$group_name}</b>.",
                "New Blog in Your Group"
            );

            insertAlert($con, $groupCreatorId, "A new blog '{$title}' posted in your group.", date("Y-m-d H:i:s"), 0);
        }
    }
...
<<<<<<< HEAD
    ...
    // At this point the forum post was created successfully.
    // Prepare notification variables
    $forumTitle = $title;
    $date = date("Y-m-d H:i:s");

    // Get group info to fetch group creator, name and slug
   ...
   $groupUrl = $siteurl . 'group/' . $group_slug;
    ...
    if ($status === "active") {
        ...
        if ($membersQ) {
            while ($m = mysqli_fetch_assoc($membersQ)) {
                ...
                // Email content
                $emailSubject = "New Blog Posted in Group";
                $emailMessage = "
                    A new blog has just been posted in <b>" . htmlspecialchars($group_name) . "</b>: <b>" . htmlspecialchars($forumTitle) . "</b>.<br><br>
                    <a href=\"" . htmlspecialchars($groupUrl) . "\">Visit the group (" . htmlspecialchars($group_name) . ") to read it.</a>
                ";

                if (!empty($memberEmail)) {
                    sendEmail($memberEmail, $siteName, $siteMail, $firstName, $emailMessage, $emailSubject);
                }

                // Insert alert for member
                $alertMessage = "A new blog titled '" . addslashes($forumTitle) . "' was posted.";
                insertAlert($con, $m['user_id'], $alertMessage, $date, 0);
            }
        }

        // -----------------------------
        // Always notify group creator
        // -----------------------------
        if ($groupCreatorId) {
            ...
            if ($creatorQ && $creator = mysqli_fetch_assoc($creatorQ)) {
                ...
                if (!empty($creatorEmail)) {
                    $subject = "New Blog in Your Group";
                    $message = "
                        A new blog was posted in your group <b>" . htmlspecialchars($group_name) . "</b>: <b>" . htmlspecialchars($forumTitle) . "</b>.<br><br>
                        <a href=\"" . htmlspecialchars($groupUrl) . "\">View the blog</a>
                    ";
                    sendEmail($creatorEmail, $siteName, $siteMail, $creatorName, $message, $subject);
                }

                // Alert for creator
                $alertMessage = "A new blog titled '" . addslashes($forumTitle) . "' was posted in your group.";
                insertAlert($con, $groupCreatorId, $alertMessage, $date, 0);
            }
        }

        // -----------------------------
        // Notify poster themselves
        // -----------------------------
        if (!empty($posterEmail)) {
            $subject = "Blog Published";
            $message = "
                Your blog <b>" . htmlspecialchars($forumTitle) . "</b> has been posted successfully in <b>" . htmlspecialchars($group_name) . "</b>.<br><br>
                <a href=\"" . htmlspecialchars($groupUrl) . "\">View the blog</a>
            ";
    ...
    // ============================
    // CASE 2: Blog is PENDING
    // ============================
    else if ($status === "pending") {

        // Determine poster's role in this group
        ...
        // If poster is NOT admin/subadmin/creator → send approval email to admins/subadmins + creator
        if (!in_array($userRole, ["admin", "subadmin"]) && $user != $groupCreatorId) {

            // Fetch all admins + subadmins
            ...
            if ($adminQ) {
                while ($a = mysqli_fetch_assoc($adminQ)) {
                    ...
                    $emailSubject = "Blog Pending Approval";
                    $emailMessage = "
                        Hello " . htmlspecialchars($adminName) . ",<br><br>
                        A blog titled <b>" . htmlspecialchars($forumTitle) . "</b> was submitted and is awaiting approval for the group <b>" . htmlspecialchars($group_name) . "</b>.<br><br>
                        <a href=\"" . htmlspecialchars($groupUrl) . "\">Review the pending blog</a>
                    ";
                    ...
                    // Alert for admin
                    $alertMessage = "Blog '" . addslashes($forumTitle) . "' is pending approval.";
                    insertAlert($con, $adminId, $alertMessage, $date, 0);
                }
            }

            // Also notify group creator (if not already)
            if ($groupCreatorId) {
                ...
                if ($creatorQ && $creator = mysqli_fetch_assoc($creatorQ)) {
                    ...
                    $emailMessage = "
                        ...
                        A blog titled <b>" . htmlspecialchars($forumTitle) . "</b> was submitted in your group <b>" . htmlspecialchars($group_name) . "</b> and is awaiting approval.<br><br>
                        <a href=\"" . htmlspecialchars($groupUrl) . "\">Review the pending blog</a>
                    ";

                    if (!empty($creatorEmail)) {
                        sendEmail($creatorEmail, $siteName, $siteMail, $creatorName, $emailMessage, $emailSubject);
                    }
