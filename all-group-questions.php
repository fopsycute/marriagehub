      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header">
            <h4 class="card-title">Group Questions</h4>
          </div>
          <div class="card-body">
            <div class="table-responsive">
              <table id="multi-filter-select" class="table table-dark">
         <thead>
          <tr>
            <th>Title</th>
            <th>Author</th>
    ...
    if (!empty($blogs)) {
        foreach ($blogs as $blog) {
            ...
            // ✅ Only display blogs where status is "pending"
             if (isset($blog->status) && $blog->group_id == $group_id) {

                  // ✅ Member only sees their own post
                if ($userRole === 'member' && $blog->user_id != $buyerId) {
                    continue;
                }
                $questionId = $blog->id;
                $title = $blog->title;
                ...
                <tr>
                    ...
                    echo "
                    <td>
                        <a href='edit-group-question.php?question_id=$questionId' class='btn btn-link btn-primary btn-lg' data-bs-toggle='tooltip' title='Edit'>
